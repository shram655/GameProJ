...
public class PlayerBuilding : MonoBehaviourPun
{
    ...
    [Tooltip("Время полного разрушения блока (сек)")]
    ...
    void Update()
    {
        ...
        bool weapon = playerController.weaponManager != null && playerController.weaponManager.HasWeaponEquipped;

        if (weapon) { HideHighlight(); CancelBreak(); return; }
        UpdateHighlight();
    ...
    // ═════════════════════════════════════════════════════
    // ⬜ БЕЛАЯ ОБВОДКА (только в радиусе разрушения = 3)
    // ═════════════════════════════════════════════════════
    void BuildHighlight()
    {
    ...
    void UpdateHighlight()
    {
        ...
        // 🆕 Обводка ТОЛЬКО в радиусе разрушения (destroyRange = 3).
        // Дальше — не загорается.
